...
let videoContestTable;
let videoContestDataTableClass;
let videoContestTableMode;
...
export function initVideoContestModule() {
    const tableEl = document.getElementById('video-contest-table');
    if (!tableEl) return;

    loadAdminDataTableLibrary().then(async (DataTable) => {
        videoContestDataTableClass = DataTable;
        await initializeVideoContestTable(tableEl);
...
export function loadVideoContests(search = '') {
    if (!videoContestTable) return;

    axios.get('/admin/video-contest', { params: { search, per_page: 200 } })
        .then(({ data }) => {
            const rows = data.data?.data ?? [];
            videoContestTable.clear();
            rows.forEach((contest) => {
                videoContestTable.row.add(buildRowData(contest));
            });
            videoContestTable.draw();
        })
        .catch(() => {
            showErrorToast('Failed to load video contests.');
        });
}

export function createVideoContest() {
    showContestForm(null);
}

export function editVideoContest(id) {
    axios.get(`/admin/video-contest/${id}`).then(({ data }) => {
...
export function removeVideoContest(id) {
    Swal.fire(buildSwalOptions({
        ...
        axios.delete(`/admin/video-contest/${id}`)
            .then(({ data }) => {
...
export function reviewSubmissions(contestId) {
    axios.get(`/admin/video-contest/${contestId}/entries`, { params: { per_page: 200 } })
        .then(({ data }) => {
...
function showContestForm(existing) {
    ...
    Swal.fire(buildSwalOptions({
        title: isEdit ? 'Edit Video Contest' : 'Open Video Contest',
        subtitle: 'Create or open a video-making contest so mobile users can submit YouTube videos for review.',
        ...
        const request = isEdit
            ? axios.put(`/admin/video-contest/${existing.id}`, value)
            : axios.post('/admin/video-contest', value);
    ...
    const rows = entries.length
        ? entries.map((entry) => `
            <div class="rounded-xl border border-slate-200 p-3 text-left">
                ...
                ${entry.video_url ? `
                    <p class="mt-2 text-xs"><a class="text-[#055498] underline" href="${escapeAttribute(entry.video_url)}" target="_blank" rel="noopener noreferrer">Open YouTube / video</a></p>
                ` : ''}
                <div class="mt-3 flex flex-wrap gap-2">
                    <button type="button" class="admin-table-action admin-table-action-primary" onclick="window.VideoContest.reviewEntry(${entry.id}, ${contest.id}, 'approved')">Approve</button>
                    <button type="button" class="admin-table-action" onclick="window.VideoContest.reviewEntry(${entry.id}, ${contest.id}, 'finalist')">Finalist</button>
                    <button type="button" class="admin-table-action admin-table-action-danger" onclick="window.VideoContest.reviewEntry(${entry.id}, ${contest.id}, 'rejected')">Reject</button>
                    <button type="button" class="admin-primary-button" style="padding:0.35rem 0.75rem;font-size:0.75rem;" onclick="window.VideoContest.setWinner(${entry.id}, ${contest.id})">Set Winner</button>
                </div>
            </div>
        `).join('')
...
export function reviewEntry(entryId, contestId, status) {
    axios.post(`/admin/video-contest/entries/${entryId}/review`, {
        status,
        admin_notes: null,
    }).then(({ data }) => {
...
export function setWinner(entryId, contestId) {
    Swal.fire(buildSwalOptions({
        ...
        if (!isConfirmed) return;
        axios.post(`/admin/video-contest/entries/${entryId}/winner`)
            .then(({ data }) => {
...
window.VideoContest = {
    create: createVideoContest,
    edit: editVideoContest,
    remove: removeVideoContest,
    reviewSubmissions,
    reviewEntry,
    setWinner,
};

async function initializeVideoContestTable(tableEl) {
    const nextMode = getTableMode();

    if (!videoContestDataTableClass) {
        return;
    }

    if (videoContestTable && videoContestTableMode === nextMode) {
        return;
    }

    if (videoContestTable) {
        videoContestTable.destroy();
        tableEl.innerHTML = '';
    }

    videoContestTableMode = nextMode;
    videoContestTable = await createAdminDataTable(tableEl, getAdminDataTableOptions({
        searchLabel: 'Search contests:',
        searchPlaceholder: 'Search video contests',
...
function bindViewportListener(tableEl) {
    ...
    const handleViewportChange = async () => {
        const nextMode = getTableMode();
        if (nextMode === videoContestTableMode) return;
        await initializeVideoContestTable(tableEl);
        loadVideoContests();
    };
...
function buildRowData(contest) {
    const actions = [
        {
            ...
            attrs: `onclick="window.VideoContest.reviewSubmissions(${contest.id})"`,
        },
        {
            tooltip: 'Edit contest',
            icon: 'fas fa-pen-to-square',
            attrs: `onclick="window.VideoContest.edit(${contest.id})"`,
        },
        {
            tooltip: 'Delete contest',
            icon: 'fas fa-trash',
            className: 'admin-table-action-danger',
            attrs: `onclick="window.VideoContest.remove(${contest.id})"`,
        },
    ];
    ...
    if (videoContestTableMode === 'mobile') {
        return [
